@extends('layouts.app', ['title' => 'kecamatan - Admin'])
    ...
    <div class="container mx-auto px-6 py-8">
        <div class="flex items-center">
            <button class="text-white focus:outline-none bg-gray-600 px-4 py-2 shadow-sm rounded-md">
                <a href="{{ route('admin.kecamatan.create') }}">TAMBAH</a>
            </button>
        ...
        <div class="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
            <div class="inline-block min-w-full shadow-sm rounded-lg overflow-hidden">
                    ...
                    @forelse($kecamatans as $kecamatan)
                        <tr class="border bg-white">
                            ...
                            <td class="px-10 py-2 text-center">
                                <a href="{{ route('admin.kecamatan.edit', $kecamatan->id_kec) }}" class="bg-indigo-600 px-4 py-2 rounded shadow-sm text-xs text-white focus:outline-none">EDIT</a>

                                <button onClick="destroy(this.id)" id="{{ $kecamatan->id_kec }}" class="bg-red-600 px-4 py-2 rounded shadow-sm text-xs text-white focus:outline-none">HAPUS</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                ...
                <div class="bg-white p-3">
                    {{ $kecamatans->links('vendor.pagination.tailwind') }}
                </div>
                @endif
            </div>
        </div>
