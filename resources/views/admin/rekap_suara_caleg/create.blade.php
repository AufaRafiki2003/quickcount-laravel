@extends('layouts.app', ['title' => 'Tambah rekap suara caleg - Admin'])

@section('content')
<main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-300">
    <div class="container mx-auto px-6 py-8">
        <div class="p-6 bg-white rounded-md shadow-md">
            <h2 class="text-lg text-gray-700 font-semibold capitalize">TAMBAH DATA REKAP SUARA CALEG</h2>
            <hr class="mt-4">
            <form action="{{ route('admin.rekap_suara_caleg.store') }}" method="POST" >
                @csrf
                    ...
                    <div class="mb-4">
                        ...
                        @foreach($calegs as $caleg)
                         <div class="w-full bg-red-200 shadow-sm rounded-md overflow-hidden mt-2">
                            <option value="{{ $caleg->id_caleg }}">{{ $caleg->nama_caleg }}</option>
                        @endforeach
                        </select>
                        @error('id_caleg')
                        <div class="w-full bg-red-200 shadow-sm rounded-md overflow-hidden mt-2">
