@extends('layouts.app', ['title' => 'caleg - Admin'])
    ...
    <div class="container mx-auto px-6 py-8">
        <div class="flex items-center">
            <button class="text-white focus:outline-none bg-gray-600 px-4 py-2 shadow-sm rounded-md">
                <a href="{{ route('admin.caleg.create') }}">TAMBAH</a>
            </button>
            <div class="relative mx-4">
                ...
                <form action="{{ route('admin.caleg.index') }}" method="GET">
            ...
            <div class="inline-block min-w-full shadow-sm rounded-lg overflow-hidden">
                        ...
                        @forelse($calegs as $caleg)
                            <tr class="border bg-white">
                                ...
                                <td class="px-10 py-2 text-center">
                                    <a href="{{ route('admin.caleg.edit', $caleg->id_caleg) }}" class="bg-indigo-600 px-4 py-2 rounded shadow-sm text-xs text-white focus:outline-none">EDIT</a>
                                    <button onClick="destroy(this.id)" id="{{ $caleg->id_caleg }}" class="bg-red-600 px-4 py-2 rounded shadow-sm text-xs text-white focus:outline-none">HAPUS</button>
                                </td>
                            </tr>
                        @empty
                            <tr>
                ...
                @if ($calegs->hasPages())
                    <div class="bg-white p-3">
                        {{ $calegs->links('vendor.pagination.tailwind') }}
                    </div>
                @endif
            </div>
        </div>
