@extends('layouts.app', ['title' => 'kelurahan - Admin'])
    ...
    <div class="container mx-auto px-6 py-8">
        <div class="flex items-center">
            <button class="text-white focus:outline-none bg-gray-600 px-4 py-2 shadow-sm rounded-md">
                <a href="{{ route('admin.kelurahan.create') }}">TAMBAH</a>
            </button>
        ...
        <div class="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
            <div class="inline-block min-w-full shadow-sm rounded-lg overflow-hidden">
                <table class="min-w-full table-auto">
                    ...
                    @forelse($kelurahans as $kelurahan)
                        <tr class="border bg-white">
                            ...
                            <td class="px-16 py-2">{{ $kelurahan->nama_kel }}</td>
                            <td class="px-16 py-2">{{ $kelurahan->kecamatans->nama_kec }}</td>

                            <!--action untuk tombol hapus dan edit belum ditambahkan -->
                            <td class="px-10 py-2 text-center">
                                <a href="{{ route('admin.kelurahan.edit', $kelurahan->id_kel) }}" class="bg-indigo-600 px-4 py-2 rounded shadow-sm text-xs text-white focus:outline-none">EDIT</a>

                                <button onClick="destroy(this.id)" id="{{ $kelurahan->id_kel }}" class="bg-red-600 px-4 py-2 rounded shadow-sm text-xs text-white focus:outline-none">HAPUS</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                ...
                <div class="bg-white p-3">
                    {{ $kelurahans->links('vendor.pagination.tailwind') }}
                </div>
                @endif
            </div>
        </div>
